...
# Die Klasse, die wir testen werden
from ts_benchmark.baselines.duet.layers.esn.reservoir_expert import UnivariateReservoirExpert
...
class TestReservoirExpert(unittest.TestCase):
    ...
    def test_01_initialization_shape_and_device(self):
        ...
        expert = UnivariateReservoirExpert(self.config).to(self.device)

        # Teste die Form der internen, nicht-trainierbaren Gewichte
        self.assertEqual(expert.W_in.shape, (self.config.reservoir_size_uni, 1), "W_in shape is incorrect.")
        self.assertEqual(expert.W_res.shape, (self.config.reservoir_size_uni, self.config.reservoir_size_uni), "W_res shape is incorrect.")
        
        # Teste die Form der trainierbaren Ausgabeschicht
        self.assertEqual(expert.readout.in_features, self.config.reservoir_size_uni, "Readout layer in_features is incorrect.")
    ...
    def test_02_initialization_hyperparams(self):
        ...
        self.assertAlmostEqual(actual_sparsity, self.config.sparsity_uni, places=2, msg="Sparsity is not correctly applied.")
        ...
        self.assertAlmostEqual(actual_spectral_radius, self.config.spectral_radius_uni, places=1, msg="Spectral radius is not correctly set.")
    ...
    def test_03_forward_pass_shape(self):
        ...
        input_tensor = torch.randn(self.B, self.L, 1).to(self.device)
        ...
        empty_input = torch.tensor([]).reshape(0, self.L, 1).to(self.device)
    ...
    def test_05_numerical_stability(self):
        ...
        expert_stable = UnivariateReservoirExpert(self.config).to(self.device)
        constant_input = (torch.ones(self.B, self.L, 1) * 5.0).to(self.device)
        ...
        chaotic_config.spectral_radius_uni = 1.2 # Macht das Reservoir instabil/chaotisch
        expert_chaotic = UnivariateReservoirExpert(chaotic_config).to(self.device)
        random_input = torch.randn(self.B, self.L, 1).to(self.device)
        ...
        extreme_input = (torch.randn(self.B, self.L, 1) * 1e6).to(self.device)
