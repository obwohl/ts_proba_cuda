        ...
        if self.norm_mode == 'identity':
            ...
            for b in range(batch_size):
                for n in range(num_features):
                    ...
                    if non_zero_values.numel() > 1: # Check if there are enough values to calculate std
                        ...
                        # If MAD is too small, use standard deviation as a fallback
                        if mad < self.eps:
                            scale = torch.std(non_zero_values)
                        else:
                            scale = mad

                        scale_stats_per_channel[b, n, 0] = scale + self.eps # Add eps for stability
                    elif non_zero_values.numel() == 1:
                        scale_stats_per_channel[b, n, 0] = torch.abs(non_zero_values[0]) + self.eps
                    else:
