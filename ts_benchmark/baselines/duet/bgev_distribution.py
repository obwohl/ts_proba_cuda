...
class BGEVDistribution(Distribution):
    ...
    def __init__(self, q_alpha, s_beta, xi, alpha=0.5, beta=0.5, p_a=0.05, p_b=0.2, c1=5, c2=5, validate_args=None):
        ...
        super().__init__(batch_shape=self.q_alpha.shape[:-1], event_shape=self.q_alpha.shape[-1:], validate_args=validate_args)

    def _l_r(self, a, xi):
        if torch.is_tensor(a) and a.min() <= 0:
            tqdm.write(f"DEBUG: _l_r input 'a' has non-positive values. Min: {a.min().item()}")
        log_a = torch.log(a)
        return (-log_a)**(-xi)

    def _l0_r(self, a):
        if torch.is_tensor(a) and a.min() <= 0:
            tqdm.write(f"DEBUG: _l0_r input 'a' has non-positive values. Min: {a.min().item()}")
        log_a = torch.log(a)
        if (log_a >= 0).any():
            tqdm.write(f"DEBUG: _l0_r input 'log_a' has non-negative values. Max: {log_a.max().item()}")
        return torch.log(-log_a)

    def _finverse_r(self, x, q_alpha, s_beta, xi, alpha, beta):
        if torch.is_tensor(x) and x.min() <= 0:
            tqdm.write(f"DEBUG: _finverse_r input 'x' has non-positive values. Min: {x.min().item()}")
    ...
    def cdf(self, y):
        ...
        z2 = torch.clamp(z2, min=-10, max=10)
        t2 = torch.exp(-z2)
    ...
    def icdf(self, prob):
        ...
        return self._finverse_r(prob_expanded, self.q_alpha.unsqueeze(-1), self.s_beta.unsqueeze(-1), self.xi.unsqueeze(-1), self.alpha, self.beta)