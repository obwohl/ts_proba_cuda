...
DATA_FILE_PATH = "preci_eisbach.csv" # KORREKTUR: Muss mit der in optuna_full_search.py verwendeten Datei übereinstimmen.
            ...
            with torch.no_grad():
                for channel_name, methods in tqdm(interesting_windows.items(), desc=f"Plots für Trial #{trial_num}", leave=False):
                    for method_name, window_start_idx in methods.items():
                        ...
                        denorm_distr, _, _, _, _, _, _, _, _ = model(input_data)
