...
class TestCRPS(unittest.TestCase):
    ...
    def test_01_deterministic_forecast_equals_mae(self):
        ...
        # Erstellt eine Verteilung, die ein scharfer Spike bei 5.0 ist.
        logits = torch.full((self.B, self.H, self.V, self.num_bins), -1e9)
        # Der Bin für den Wert 5.0 ist bei Index 75 ((-10 bis 10, 100 Bins) -> (5 - (-10)) / 20 * 100 = 75)
        logits[:, :, :, 75] = 1e9 
        # Macht die Tails irrelevant
        dist = self._create_dist(logits, lower_beta=1e-9, upper_beta=1e-9)

        # Der Zielwert ist 8.0
        target = torch.full((self.B, self.V, self.H), 8.0, device=self.device)
        
        # Berechne CRPS und MAE
        crps_val = crps_loss(dist, target).mean()
        mae_val = torch.abs(dist.mean - target.permute(0, 2, 1)).mean()

        self.assertAlmostEqual(crps_val.item(), mae_val.item(), places=4,
                               msg=f"Für eine deterministische Vorhersage sollte CRPS ({crps_val.item()}) gleich MAE ({mae_val.item()}) sein.")
    ...
    def test_02_perfect_median_forecast(self):
        ...
        # Eine symmetrische Verteilung, zentriert um 5.0
        logits = torch.full((self.B, self.H, self.V, self.num_bins), 0.0)
        logits[:, :, :, 74] = 10.0
        logits[:, :, :, 75] = 10.0
        dist = self._create_dist(logits)

        # Zielwert ist ebenfalls 5.0
    ...
    def test_03_high_uncertainty_vs_high_bias(self):
        ...
        # --- Fall A: Hohe Unsicherheit (breite Verteilung) ---
        # Eine sehr flache, breite Verteilung um 5.0
        logits_wide = torch.full((self.B, self.H, self.V, self.num_bins), 1.0)
        ...
        # --- Fall B: Hoher Bias (schmale, aber falsche Verteilung) ---
        # Eine sehr scharfe Verteilung, zentriert bei 2.0
        logits_narrow = torch.full((self.B, self.H, self.V, self.num_bins), -1e9)
        logits_narrow[:, :, :, 60] = 1e9 # Bin für 2.0
    ...
    def test_04_extreme_value_in_tail(self):
        ...
        logits = torch.full((self.B, self.H, self.V, self.num_bins), -1e9)
        logits[:, :, :, 50] = 1e9 # Spike bei 0.0
        dist = self._create_dist(logits)
        
        crps_normal = crps_loss(dist, torch.full_like(logits[..., 0], 0.5)).mean()
        crps_extreme = crps_loss(dist, torch.full_like(logits[..., 0], 50.0)).mean()
    ...
    def test_05_nan_inf_robustness(self):
        ...
        dist = self._create_dist(torch.full((self.B, self.H, self.V, self.num_bins), 0.0))
        target_bad = torch.full((self.B, self.V, self.H), 5.0, device=self.device)
        target_bad[0, 0, 0] = float('nan')
        target_bad[0, 0, 1] = float('inf')
        
        crps_val = crps_loss(dist, target_bad).mean()

        self.assertFalse(torch.isnan(crps_val), "CRPS sollte nicht NaN sein, wenn der Zielwert NaN enthält.")
        self.assertFalse(torch.isinf(crps_val), "CRPS sollte nicht Inf sein, wenn der Zielwert Inf enthält.")
